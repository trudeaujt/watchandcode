...
(function(root, undefined) {
	...
	function isString(obj) {
		return !!(obj === '' || (obj && obj.charCodeAt && obj.substr)); //equivalent to Boolean(...). Converts stuff in the () to a boolean value.
	}
	 ...
	 * function		returns a string or object as above
	 * 
	 * Returns:
	 * object		has "pos(required, must contain "%v"), neg, zero" properties
	 * 
	...
	function checkCurrencyFormat(format) {
		...
		if ( isString( format ) && format.match("%v") ) {
			...
			return {
				...
				zero : format // in case you want zero to show up differently than other numbers
			};
	...
	function betterToFixed(value, precision) { //using exponential notation
		//1.005e2
		var exponentialForm = Number(value + 'e' + precision);
		var rounded = Math.round(exponentialForm);
		var finalResult = Number(rounded + 'e-' + precision);
		return finalResult.toFixed(precision); // turn back into string
	}
